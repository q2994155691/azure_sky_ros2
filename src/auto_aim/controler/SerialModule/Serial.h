...
namespace rm
{
	...
	static uint8_t getCRC8(uint8_t* data, int dwLength)
	{
	...
	static uint16_t getCRC16(uint8_t* data, int length)
	{
		auto _data = reinterpret_cast<unsigned char*>(data);
		uint16_t ucCRC16 = 0xffff;
		unsigned char ucIndex;
		while (length--)
		{
			ucIndex = (ucCRC16 ^ (*_data++)) & 0x00ff;
			ucCRC16 = ((ucCRC16 >> 8) ^ CRC16_TAB[ucIndex]);
		}
		return ucCRC16;
	};
	...
	{
	public:
		// 读取数据包,当前接收类不含有长度,只有头帧,尾帧和CRC16校验
