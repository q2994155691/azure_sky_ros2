...
class AdaptiveEKF {
    using MatrixXX = Eigen::Matrix<float, N_X, N_X>;
    using MatrixYX = Eigen::Matrix<float, N_Y, N_X>;
    using MatrixXY = Eigen::Matrix<float, N_X, N_Y>;
    using MatrixYY = Eigen::Matrix<float, N_Y, N_Y>;
    using VectorX = Eigen::Matrix<float, N_X, 1>;
    using VectorY = Eigen::Matrix<float, N_Y, 1>;
...
public:
    ...
    VectorX predict(Func &&func) {
        ceres::Jet<float, N_X> Xe_auto_jet[N_X];
        for (int i = 0; i < N_X; i++) {
            Xe_auto_jet[i].a = Xe[i];
            Xe_auto_jet[i].v[i] = 1;
        }
        ceres::Jet<float, N_X> Xp_auto_jet[N_X];
    ...
    VectorX update(Func &&func, const VectorY &Y) {
        ...
        ceres::Jet<float, N_X> Yp_auto_jet[N_Y];
